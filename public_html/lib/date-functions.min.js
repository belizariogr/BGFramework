...
function convertLocalDateToUTCDate(d) {var nD = new Date(d.getTime()+d.getTimezoneOffset()*60*1000);return nD;}
...
function firstDayOfMonth(date,format){if(date instanceof Date)var d=date;else{if(!date || !isDate(date, format))return;var d=new Date(getDateFromFormat(date,format));}var r = {y: d.getFullYear(),m:d.getMonth()};return new Date(r.y, r.m, 1);}
function lastDayOfMonth(date,format){if(date instanceof Date)var d=date;else{if(!date || !isDate(date, format))return;var d=new Date(getDateFromFormat(date,format));}var r = {y: d.getFullYear(),m:d.getMonth()};d=new Date(r.y, r.m, 1);d.setMonth(d.getMonth()+1);d.setDate(d.getDate()-1);return d;}
